...
import {
  ...
  Star, Trash2, AlertTriangle, Users,
...
import { CompanyEnrichmentModal, type CompanyEnrichmentDecision } from './CompanyEnrichmentModal'
...
// ─── Stable-order hook — rows never jump when a lead is updated ───────────────
/**
 * Maintains a stable display order: new leads prepend, deleted leads remove,
 * updates NEVER change position. Row order only changes on filter changes.
 */
function useStableLeads(leads: Lead[]): Lead[] {
  ...
  return useMemo(() => {
    ...
    // Find leads that are brand-new (not in stable order yet)
    const newIds = leads
      .map(l => l.id)
      .filter(id => !stableIdsRef.current.includes(id))

    // Rebuild stable ID list: new leads at top, existing in order, deleted removed
    stableIdsRef.current = [
      ...newIds,
      ...stableIdsRef.current.filter(id => currentIds.has(id)),
    ]
...
function EditableLinkCell({
  url, onSave, placeholder, icon: Icon,
}: {
  url: string | null
...
export function LeadTable({ leads, isLoading, onRowClick }: LeadTableProps) {
  const { updateLead, deleteLead, bulkUpdate } = useLeads()
  const [selected, setSelected] = useState<string[]>([])
  const [page, setPage] = useState(0)

  // Reset page when leads change (filter change)
  const prevLeadsLengthRef = useRef(leads.length)
  useEffect(() => {
    if (leads.length !== prevLeadsLengthRef.current) {
      prevLeadsLengthRef.current = leads.length
    }
  }, [leads.length])

  // Stable ordering — no row jumps on update
  const stableLeads = useStableLeads(leads)
  ...
  // Old company enrichment (website + linkedin via scraping)
  const [enrichModalOpen, setEnrichModalOpen]         = useState(false)
  const [isEnriching, setIsEnriching]                 = useState(false)
  const [enrichmentResults, setEnrichmentResults]     = useState<EnrichmentResult[]>([])
  const [enrichingLabel, setEnrichingLabel]           = useState('')

  // NEW: LinkedIn company finder modal
  const [liEnrichOpen, setLiEnrichOpen] = useState(false)
  const [liEnrichLeads, setLiEnrichLeads] = useState<Lead[]>([])
  ...
  const cancelEdit = () => {
    ...
    setEditingValue(''); setOriginalValue('')
  }
  ...
  const startEdit = (leadId: string, field: string, currentValue: string | null | undefined) => {
    ...
    setEditingId(leadId); setEditingField(field)
    setEditingValue(val); setOriginalValue(val)
  }

  const handleKeyDown = (
    e: React.KeyboardEvent, leadId: string, field: string,
    currentValue: string, originalVal: string,
  ) => {
  ...
  // ── Old enrichment (website + scrape linkedin) ────────────────────────────

  const handleEnrichSelected = async () => {
    if (selected.length === 0) return
    setEnrichingLabel(
      selected.length === 1
        ? `Searching data for ${stableLeads.find(l => l.id === selected[0])?.companyName ?? 'company'}…`
    ...
    try {
      const selectedLeads = stableLeads.filter(l => selected.includes(l.id))
      ...
      const results: EnrichmentResult[] = selectedLeads.map(lead => {
        ...
        return {
          ...
          existingLinkedin: lead.companyLinkedinUrl ?? null,
        }
      })
      ...
      setEnrichmentResults(
        stableLeads
          ...
          .map(l => ({
            leadId: l.id, companyName: l.companyName,
            website: null, linkedinUrl: null, confidence: 0, source: 'none' as const,
            existingWebsite: l.companyWebsite ?? null, existingLinkedin: l.companyLinkedinUrl ?? null,
          })),
      )
    } finally {
  ...
  const handleEnrichConfirm = (decisions: EnrichmentDecision[]) => {
    for (const d of decisions) {
      ...
      if (d.acceptLinkedin && result.linkedinUrl) updates.companyLinkedinUrl = result.linkedinUrl
  ...
  // ── NEW: LinkedIn company enrichment ──────────────────────────────────────

  const handleLinkedInEnrichSelected = () => {
    const sel = stableLeads.filter(l => selected.includes(l.id))
    setLiEnrichLeads(sel)
    setLiEnrichOpen(true)
  }

  const handleLinkedInEnrichConfirm = (decisions: CompanyEnrichmentDecision[]) => {
    const accepted = decisions.filter(d => d.accept)
    if (accepted.length > 0) {
      const updates = accepted.map(d => {
        const u: Partial<Lead> = {}
        if (d.linkedinUrl)   u.companyLinkedinUrl = d.linkedinUrl
        if (d.websiteUrl)    u.companyWebsite     = d.websiteUrl
        if (d.industry)      u.companyIndustry    = d.industry
        if (d.companyPhone)  u.contactPhone       = d.companyPhone
        if (d.employeeCount != null)
          u.companyEmployeeCount = String(d.employeeCount)
        return { id: d.leadId, updates: u }
      })
      bulkUpdate(updates)
    }
    setLiEnrichOpen(false)
    setLiEnrichLeads([])
    setSelected([])
  }
  ...
  const totalPages = Math.ceil(stableLeads.length / PAGE_SIZE)
  const paginated  = stableLeads.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE)
  ...
  const handleBulkEmailStatus = (status: LeadStatus) => {
    ...
    for (const id of selected) {
      ...
      if (['Email 1', 'Email 2', 'Email 3'].includes(status)) {
        updates.lastActionDate = now
        updates.nextActionDate = calculateNextActionDate(now)
        updates.nextActionDays = 5
      }
      if (['Closed', 'Sequence Closed'].includes(status)) {
        updates.nextActionDate = null
        updates.nextActionDays = null
      }
  ...
  const handleBulkResponse = (response: ResponseStatus) => {
    for (const id of selected) {
      const lead = stableLeads.find(l => l.id === id)
      if (!lead) continue
      const clean = getCleanComments(lead.opsComments)
      const text  = response !== 'none' ? `[Response: ${response}] ${clean}` : clean
      updateLead({ id, updates: { opsComments: text.trim() || null, response: response === 'none' ? null : response } })
    }
    setSelected([])
  }
  ...
  const renderEditableCell = (
    ...
    const isEdit = editingId === leadId && editingField === field
    if (isEdit) {
      return (
        <div className="flex items-center gap-1 min-w-[200px]">
          ...
          />
          <button
            onClick={e => { e.stopPropagation(); handleSave(leadId, field, editingValue, originalValue) }}
  ...
  if (stableLeads.length === 0) return (
    <EmptyState title="No leads found" description="Try adjusting your filters or run the scraper to find new leads" />
  )

  return (
    <div className="flex flex-col gap-3">

      {/* ── Company Website Enrichment Modal (old) ── */}
      ...
      {/* ── LinkedIn Company Finder Modal (NEW) ── */}
      <CompanyEnrichmentModal
        isOpen={liEnrichOpen}
        leads={liEnrichLeads}
        onConfirm={handleLinkedInEnrichConfirm}
        onClose={() => { setLiEnrichOpen(false); setLiEnrichLeads([]) }}
      />
      ...
      {selected.length > 0 && (
        <div className="sticky top-0 z-10 flex flex-wrap items-center gap-2 px-4 py-2.5 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-xl shadow-sm">
          ...
          <div className="flex gap-1">
            {/* NEW: LinkedIn Company Finder */}
            <Button
              variant="outline" size="sm"
              onClick={handleLinkedInEnrichSelected}
              leftIcon={<Linkedin className="w-3 h-3 text-[#0077B5]" />}
              className="h-7 px-2 text-xs border-blue-200 text-[#0077B5] hover:bg-blue-50"
            >
              Find LinkedIn
            </Button>

            {/* Old website enrichment */}
            <Button
              variant="outline" size="sm"
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700">
              ...
              {[
                ...
                'Phone', 'Email', 'Person LinkedIn', 'Company LinkedIn',  // ← renamed + new
                'Company URL', 'Match Assessment',
                ...
                <th
                  ...
                  className={cn(
                    ...
                    (h === 'Person LinkedIn' || h === 'Company LinkedIn') && 'bg-blue-50/50 dark:bg-blue-900/10',
                  )}
                >
                  {h}
                </th>
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
            {paginated.map(lead => {
              const response = lead.response
                ? (lead.response as ResponseStatus)
                : getResponseFromComments(lead.opsComments)
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3 min-w-[140px]">
                    {renderEditableCell(lead.id, 'contactName', lead.contactName, 'Add name')}
                  </td>

                  {/* Role Title */}
                  <td className="px-4 py-3 min-w-[140px]">
                    {renderEditableCell(lead.id, 'contactRole', lead.contactJobTitle, 'Add role')}
                  </td>

                  {/* Phone */}
                  <td className="px-4 py-3 min-w-[130px]">
                    {renderEditableCell(lead.id, 'phone', lead.contactPhone, 'Add phone', 'tel')}
                  </td>

                  {/* Email */}
                  <td className="px-4 py-3 min-w-[180px]">
                    {renderEditableCell(lead.id, 'email', lead.contactEmail, 'Add email', 'email')}
                  </td>

                  {/* ── Person LinkedIn (was "LinkedIn") ── */}
                  <td className="px-4 py-3 min-w-[160px] bg-blue-50/20 dark:bg-blue-900/5">
                    <EditableLinkCell
                      url={lead.contactLinkedinUrl}
                      onSave={value => updateLead({ id: lead.id, updates: { contactLinkedinUrl: value || null } })}
                      placeholder="Add person LinkedIn"
                      icon={Linkedin}
                    />
                  </td>

                  {/* ── Company LinkedIn (NEW) ── */}
                  <td className="px-4 py-3 min-w-[160px] bg-blue-50/20 dark:bg-blue-900/5">
                    <EditableLinkCell
                      url={lead.companyLinkedinUrl}
                      onSave={value => updateLead({ id: lead.id, updates: { companyLinkedinUrl: value || null } })}
                      placeholder="Add company LinkedIn"
                      icon={Linkedin}
                    />
                  </td>
                  ...
                  <td className="px-4 py-3 min-w-[110px]" onClick={e => e.stopPropagation()}>
                    {editingId === lead.id && editingField === 'matchAssessment' ? (
                      <div className="flex items-center gap-1 min-w-[150px]">
                        ...
                        <button onClick={e => { e.stopPropagation(); handleSave(lead.id, 'matchAssessment', editingValue, originalValue) }} className="p-1 text-green-600 hover:text-green-700 rounded hover:bg-green-50 shrink-0">
                          <Check className="w-4 h-4" />
                        </button>
                        <button onClick={e => { e.stopPropagation(); cancelEdit() }} className="p-1 text-red-600 hover:text-red-700 rounded hover:bg-red-50 shrink-0">
                          <X className="w-4 h-4" />
                        </button>
                      </div>
                    ) : (
                  ...
                  <td className="px-4 py-3 min-w-[120px]" onClick={e => e.stopPropagation()}>
                    <select
                      ...
                      onChange={e => {
                        ...
                        updateLead({ id: lead.id, updates: { opsComments: text.trim() || null, response: r === 'none' ? null : r } })
                      }}
                      className={cn(
                        'px-2 py-1.5 text-sm rounded-lg border cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-1 font-medium w-full',
                        'bg-white dark:bg-slate-800',
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-2">
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Showing {page * PAGE_SIZE + 1}–{Math.min((page + 1) * PAGE_SIZE, stableLeads.length)} of {stableLeads.length} leads
          </p>
