        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center shadow-sm ${
              isDone || error || isRunning ? 'bg-white/15 backdrop-blur' : 'bg-slate-200 dark:bg-slate-700'
            }`}>
              {isDone ? (
            ...
            <div>
              <h3 className={`font-semibold ${isDone || error || isRunning ? 'text-white' : 'text-slate-900 dark:text-white'}`}>
                {isDone ? 'Search Complete ✨' : error ? 'Error Occurred' : isRunning ? 'Scraping in Progress' : 'Scraper Status'}
              </h3>
              <p className={`text-xs mt-0.5 ${isDone || error || isRunning ? 'text-white/70' : 'text-slate-500 dark:text-slate-400'}`}>
                {isDone
          ...
          {(isRunning || (isDone && metrics.elapsedSeconds > 0)) && (
            <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full ${
              isDone || isRunning ? 'bg-white/15 backdrop-blur' : 'bg-slate-100 dark:bg-slate-800'
            }`}>
              <Clock className={`w-3.5 h-3.5 ${isDone || isRunning ? 'text-white/80' : 'text-slate-500'}`} />
              <span className={`text-sm font-mono font-bold ${isDone || isRunning ? 'text-white' : 'text-slate-700 dark:text-slate-300'}`}>
                {liveSeconds}s
              </span>
            </div>
          )}
      ...
      <CardBody className="space-y-5">
            ...
            {STEPS.map((step, index) => {
              ...
              return (
                <div key={step.id} className="relative">
                  {!isLast && (
                    <div
                      className={`absolute left-[18px] top-9 w-0.5 h-5 transition-colors duration-500 ${
                        status === 'complete'
                  ...
                  <div className={`flex items-start gap-3 p-2 rounded-xl transition-colors duration-300 ${
                    status === 'active' ? 'bg-blue-50 dark:bg-blue-900/20' : ''
                  }`}>
                    <div className={`shrink-0 w-9 h-9 rounded-full flex items-center justify-center mt-0.5 transition-all duration-300 ${
                      status === 'complete'
                        ? 'bg-emerald-100 dark:bg-emerald-900/40'
                        : status === 'active'
                        ? 'bg-blue-100 dark:bg-blue-900/40 ring-2 ring-blue-300 dark:ring-blue-600 ring-offset-1'
                        : status === 'error'
                        ? 'bg-red-100 dark:bg-red-900/40'
                        : 'bg-slate-100 dark:bg-slate-800'
                    }`}>
                      {status === 'complete' && <CheckCircle2 className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />}
                      {status === 'active' && <Loader2 className="w-4 h-4 text-blue-600 dark:text-blue-400 animate-spin" />}
                      {status === 'error' && <AlertCircle className="w-4 h-4 text-red-500" />}
                      {status === 'pending' && <Circle className="w-4 h-4 text-slate-300 dark:text-slate-600" />}
                    </div>
        ...
        {(metrics.totalRaw > 0 || isDone) && (
          <div className="grid grid-cols-3 gap-3 pt-2 border-t border-slate-200 dark:border-slate-700">
            <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-200 dark:border-blue-800 text-center">
              <p className="text-2xl font-black text-blue-700 dark:text-blue-300 tabular-nums">{metrics.totalRaw}</p>
              <p className="text-xs font-medium text-blue-600 dark:text-blue-400 mt-0.5">Jobs Found</p>
            </div>
            <div className="p-3 bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-200 dark:border-red-800 text-center">
              <p className="text-2xl font-black text-red-700 dark:text-red-300 tabular-nums">{metrics.filteredOut}</p>
              <p className="text-xs font-medium text-red-600 dark:text-red-400 mt-0.5">Filtered Out</p>
            </div>
            <div className="p-3 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-200 dark:border-emerald-800 text-center">
              <p className="text-2xl font-black text-emerald-700 dark:text-emerald-300 tabular-nums">{metrics.validLeads}</p>
              <p className="text-xs font-medium text-emerald-600 dark:text-emerald-400 mt-0.5">Valid Leads</p>
            </div>
          </div>
        )}
        ...
        {isIdle && !error && (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            <div className="w-16 h-16 bg-gradient-to-br from-blue-100 to-indigo-100 dark:from-slate-800 dark:to-slate-700 rounded-2xl flex items-center justify-center mb-4 shadow-sm">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-0.5">
                  {metrics.validLeads} valid leads from {metrics.totalRaw} Sales jobs
                  {metrics.filteredOut > 0 && ` · ${metrics.filteredOut} agencies/firms removed`}
                </p>
              </div>
            </div>
          </div>
